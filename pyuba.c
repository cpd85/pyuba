...
new_pyuba_method(PyObject *self, PyObject *args)
{
  ...
  if (PyArg_ParseTuple(args, "i", &nitems) == 0) {
    return NULL;
  }

  /* Prepare pyuba structure */
  pyuba_struct mystruct;
  mystruct.array = malloc(sizeof(int) * 2 * nitems);
  mystruct.size = 2 * nitems;
  mystruct.numitems = nitems;
  mystruct.start = mystruct.array;
  mystruct.end = mystruct.end;
  

  return Py_BuildValue("i", (long) &mystruct);
}
